...
# print(model(torch.zeros((128, 20, 136)), torch.zeros((128, 20))).size())
...
def q_sample(x0, t, scheduler, noise=None):
	...
	x_t = torch.sqrt(alpha_bar).unsqueeze(-1) * x0 + torch.sqrt(1 - alpha_bar).unsqueeze(-1) * noise
...
#         t = torch.tensor([t], device=device).unsqueeze(1)
#         x = model(x, t)
...
for epoch in range(1000):
	...
	for i, sample_i in enumerate(samples):
		...
		np.savez(f"output/output_{step}_{i}.npz", sequence_shape=basic_shape, sequence_mhr_model_params_latent=mhr, sequence_expr_params=basic_expr)
